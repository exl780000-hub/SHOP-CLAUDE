...
- 工資試算升級：輸入客戶名稱存成一筆、未結算清單勾選多筆結算總額、已結算收合區（含合計/退回/刪除）
- 工資試算紀錄改存專用 Notion 資料庫「🧮 工資試算紀錄」（id 12cc4105-77c2-48f6-b702-443910366a66），CRUD 併入 wage-settlement API（?calc=1 / action:calc-*），跨裝置共用；若 API 回權限錯誤需在 Notion 將該資料庫分享給 Integration
- 工資試算改版：頂層三分頁（外套師傅/褲子師傅/經理），師傅內選項目（外套師傅：外套/修改/背心 2200/2500；褲子師傅：褲子/背心 1900），清單與結算按師傅隔離（Notion 類型欄=師傅名）；修改項目僅外套類（肩領/袖子/腰身/袖長），未結算清單有全選
