...
export default function Wages() {
  ...
  const [filterFlow, setFilterFlow] = useState("進行中");
  ...
  const filtered = orders.filter(o => {
    if (filterFlow === "進行中") return o.flow !== "✅ 完成";
    if (filterFlow === "✅ 完成") return o.flow === "✅ 完成";
    return true;
  }).filter(o => !search || o.name.includes(search));

  const totalWage = filtered.reduce((s, o) => s + (o.totalWage || 0), 0);
    ...
    <div style={{ maxWidth: 520, margin: "0 auto", padding: "14px 14px 80px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="搜尋客戶名稱"
          style={{ flex: 1, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "9px 14px", color: C.ivory, fontSize: 14, outline: "none" }} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, alignItems: "center" }}>
        {["進行中", "✅ 完成", "全部"].map(f => (
          <button key={f} onClick={() => setFilterFlow(f)} style={{
            ...
            border: `1px solid ${filterFlow === f ? C.gold : C.border}`,
            background: filterFlow === f ? C.gold + "22" : "transparent",
            color: filterFlow === f ? C.gold : C.sage,
          }}>{f}</button>
        ))}
        <div style={{ marginLeft: "auto", fontSize: 12, color: C.sage }}>
          工資合計 <span style={{ color: C.gold, fontWeight: 700, fontFamily: "Georgia,serif" }}>${totalWage.toLocaleString()}</span>
        </div>
      </div>

      {loading && <div style={{ color: C.sage, textAlign: "center", padding: 30 }}>載入中...</div>}
      {!loading && filtered.length === 0 && <div style={{ color: C.sage, textAlign: "center", padding: 40 }}>沒有訂單</div>}
